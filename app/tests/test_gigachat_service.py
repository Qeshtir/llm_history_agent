...
def test_generate_response(gigachat_service):
    ...
    assert re.search(r'\bмай\b|\bмая\b', response.lower())  # Проверяем наличие "май" или "мая"
...
def test_generate_response_no_context(gigachat_service):
    ...
    assert "нет информации" in response.lower() or "не могу ответить" in response.lower()