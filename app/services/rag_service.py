...
class RAGService:
    ...
    def load_documents_from_directory(self, docs_dir: str = "/app/data/scratches/cleaned_docs", json_file: str = "urls.json", collection_name: str = collection_name):
        ...
        # FIXME: использовать встроенный текстово-структурный сплитер (делит по абзацам, строкам, словам)
        # from langchain_text_splitters import RecursiveCharacterTextSplitter
        # text_splitter = RecursiveCharacterTextSplitter(chunk_size=100, chunk_overlap=0)
        # texts = text_splitter.split_text(document)
        # https://python.langchain.com/docs/concepts/text_splitters/#text-structured-based
        # или семантический сплитер (делит по смыслам)
        ...
        for doc in urls:
            ...
            # if self.chroma_service.document_exists(collection_name, str(file_path)):
            #     logger.info(f"Документ уже существует в коллекции: {file_path}")
            #     continue
            # FIXME: добавить проверку на дублирование документов, Например, по метаданным  
            ...
            try:
                ...
                logger.info(f"Документы успешно загружены в коллекцию {collection_name}")
    ...
    def generate_answer(self, query: str, collection_name: str) -> str:
        ...
        results = self.chroma_service.query_documents(collection_name=collection_name, query_text=query, n_results=5)
        
        # Извлечение контекста
        context = [doc[0] for doc in results['documents']]  # Извлекаем текст из каждого документа
